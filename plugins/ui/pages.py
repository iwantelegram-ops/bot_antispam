...
from database import get_config, db, get_group_action_log_page, TZ_WIB as _TZ_WIB
    ...
    try:
        if _CHANNEL_OWNER:
            ...
            if getattr(ch, "username", None):
                parts.append(f'📢 <a href="https://t.me/{ch.username}">{ch_title}</a>')
            else:
...
async def page_regex_list(chat_id: int):
    docs = [doc async for doc in group_regex_db.find({"chat_id": chat_id})]

    if docs:
        lines = "\n".join(
            f"  <b>{i+1}.</b> <code>{doc.get('raw', '—')}</code>"
            for i, doc in enumerate(docs)
        )
        body = (
            f"⚡ <b>Aktif:</b> <code>{len(docs)} pola</code>\n\n"
            f"{lines}\n\n"
            f"<i>Tap 🗑 di bawah untuk hapus filter secara instan.</i>"
        )
        del_buttons = [
            [InlineKeyboardButton(
                f"🗑  {doc.get('raw', doc['pattern'])[:35]}",
                callback_data=f"rgxdel_{chat_id}_{i}"
            )]
            for i, doc in enumerate(docs)
        ]
    else:
