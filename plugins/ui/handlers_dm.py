...
─────────────────────────
Handler untuk interaksi di DM (private chat):
  - /start, /antigcast → tampilkan menu utama
  - Semua callback_query (navigasi halaman, toggle, dll)

Bug fixes:
  - edit_with_bq: hapus collapsed=True dari _RawBQ (tidak didukung Pyrogram 2.0.106)
  - cb_grp_log: gunakan safe_edit biasa (HTML blockquote, tanpa raw API)
  - cb_regex_del / cb_free_del: tambah cb.answer() di awal sebelum operasi DB
  - Guard MessageNotModified & semua Exception di semua callback
"""
    ...
    for token in SPLIT_RE.split(text):
        if token == "[B]":
            ...
            if bq_start is not None:
                ...
                if length > 0:
                    # FIXED: Hapus collapsed=True — tidak didukung Pyrogram 2.0.106
...
async def cb_grp_log(client, cb: CallbackQuery):
    # FIXED: answer() dipanggil PERTAMA agar spinner tidak macet
    await cb.answer()
    try:
        ...
        # FIXED: Gunakan safe_edit biasa — page_group_log sekarang return HTML murni
        await safe_edit(cb.message, text, keyboard)
    except Exception as e:
...
@Client.on_callback_query(filters.regex(r"^rgxlist_(-?\d+)$"))
async def cb_regex_list(client, cb: CallbackQuery):
    await cb.answer()
    clear_all_fsm(cb.from_user.id)
    chat_id = int(re.search(r"(-?\d+)$", cb.data).group(1))
    text, keyboard = await page_regex_list(chat_id)
    await safe_edit(cb.message, text, keyboard)
...
@Client.on_callback_query(filters.regex(r"^rgxdel_(-?\d+)_(\d+)$"))
async def cb_regex_del(client, cb: CallbackQuery):
    # FIXED: answer() dipanggil PERTAMA agar Telegram tidak timeout
    await cb.answer("⏳ Menghapus...")
    try:
        m       = re.match(r"^rgxdel_(-?\d+)_(\d+)$", cb.data)
        chat_id = int(m.group(1))
        index   = int(m.group(2))

        from database import db
        _group_regex_db = db["regex_per_group"]
        docs = [doc async for doc in _group_regex_db.find({"chat_id": chat_id})]
        if index < len(docs):
            await _group_regex_db.delete_one({"_id": docs[index]["_id"]})
            await cb.answer("✅ Filter berhasil dihapus.", show_alert=False)
        else:
            await cb.answer("⚠️ Sinkronisasi gagal: Data tidak ditemukan.", show_alert=True)
...
async def cb_free_del(client, cb: CallbackQuery):
    ...
    try:
        ...
        if index < len(docs):
            ...
            await cb.answer("✅ User berhasil dihapus dari daftar VIP.", show_alert=False)
        else:
            await cb.answer("⚠️ Data tidak ditemukan.", show_alert=True)
