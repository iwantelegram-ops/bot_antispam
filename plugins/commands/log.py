...
from database import (
    ...
    GLOBAL_EXPIRY, TZ_WIB, insert_group_action_log,
)
...
async def _send_log(client: Client, text: str) -> bool:
    ...
    """
    global _log_channel_valid
    if not LOG_CHANNEL:
        return False
    if _log_channel_valid is False:
        return False  # Sudah diketahui invalid, jangan coba lagi
    try:
        ...
        if _log_channel_valid is None:  # Hanya print 1 kali saat pertama kali gagal
            print(f"[LOG] LOG_CHANNEL tidak valid: {e}. Pastikan LOG_CHANNEL benar di .env")
...
async def log_deletion_trigger(client: Client, message: Message):
    ...
    # Gcast: simpan ke group log
    if alasan == "Anti-Broadcast Gcast Global":
        try:
            await insert_group_action_log(
                cid, "HAPUS", "Broadcast ke banyak grup sekaligus",
                uid, message.from_user.first_name or str(uid),
                content[:100],
            )
        except Exception:
            pass
...
async def log_spam_global(client: Client, message: Message, pola: str, indikator: str):
    ...
    try:
        await insert_group_action_log(
            cid, "HAPUS", "Filter AI global (Nexus)",
            uid, message.from_user.first_name or str(uid), "",
        )
    except Exception:
        pass
...
async def log_spam_lokal(client: Client, message: Message, pola: str, indikator: str):
    ...
    try:
        await insert_group_action_log(
            cid, "HAPUS", "Filter kata manual owner (Nexus)",
            uid, message.from_user.first_name or str(uid), "",
        )
    except Exception:
        pass
