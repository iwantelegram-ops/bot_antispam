...
async def _migrate_legacy_data():
    ...
    _COLLECTIONS = [
        ...
        "ai_debug_log", "dm_users", "nexus_ai_model",
    ]
...
async def setup_db():
    ...
    if _BACKEND == "sqlite":
        ...
        for col_name in [
            ...
            "nexus_actlog", "ai_debug_log", "dm_users", "nexus_ai_model",
        ]:
...
async def get_my_admin_groups(client, user_id: int) -> list:
    """
    Kembalikan semua grup dari config_db (berbagi via CODE_BOT).
    ...
    async for doc in config_db.find({}):
        ...
        if not chat_id:
            continue
        # Gunakan judul tersimpan terlebih dahulu (works across bots with same CODE_BOT)
        ...
        try:
            ...
            # Perbarui judul tersimpan di database
            await config_db.update_one(
                {"chat_id": chat_id},
                {"$set": {"title": title}},
            )
        except Exception:
            # Bot token ini tidak ada di grup — gunakan judul tersimpan dari DB
            # Grup ini mungkin dikelola oleh bot lain dengan CODE_BOT yang sama
            if not doc.get("title"):
                continue  # belum ada judul tersimpan sama sekali, lewati
